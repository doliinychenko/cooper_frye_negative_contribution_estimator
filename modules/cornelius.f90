!  cornelius version 1.4: Copyright 2012, Pasi Huovinen
!
...
! A PARTICULAR PURPOSE.
   ...
   SUBROUTINE Cornelius(E0,HyperCube,dSigma,Nsurf,Vmid,dt,dx,dy,dz,  &
                        Nambi,Ndisc)
!
!  version 1.4 zeta
!
...
! - added the license statement and a test that the hypercube really contains
!   a surface elements. Ngp no longer required when the routine is called.
...
!
     ...
     Ngp = COUNT(HyperCube .ge. E0)
     dSigma = 0D0

! Check that the hypercube contains a surface
...
!  of corner coordinates)
     ...
     DO i = First,Last
        ...
        CALL TetraVolume(A,B,C,SuL) !Covariant components of the surface normal

        Vout = Out(:,i) - Vmids(:,ns) ! Direction outwards, to lower energy
